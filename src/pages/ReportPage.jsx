...
export default function ReportPage({ records, employees }) {
  const now = new Date();
    ...
    <div className="report-page">
      <div className="report-header">
        ...
        <div className="report-month-nav">
          <button className="rpt-nav-btn" onClick={() => changeMonth(-1)}>&#8592;</button>
          <span className="rpt-month-label">{MONTHS_TH[month]} {year + 543}</span>
          <button className="rpt-nav-btn" onClick={() => changeMonth(1)}>&#8594;</button>
        </div>
      </div>
          ...
          <tbody>
            {rows.map((e) => (
              <tr key={e.id}>
                ...
                <td><span className="rpt-ok">{e.onTime}</span></td>
                <td><span className="rpt-late">{e.late}</span></td>
